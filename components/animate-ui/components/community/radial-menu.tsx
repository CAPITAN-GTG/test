...
function RadialMenu({
  ...
  const handleSelect = React.useCallback((item: MenuItem) => {
    onSelect?.(item);
    // Close menu by clicking outside (Radix UI will handle it)
    // We can also dispatch an escape key event to close
  ...
  return (
    <ContextMenu.Root>
      <ContextMenu.Trigger asChild className="w-full h-full">
        <div className={cn('select-none outline-none w-full h-full')}>
          {children}
        </div>
      </ContextMenu.Trigger>
      ...
      <ContextMenu.Portal>
        <ContextMenu.Content
          className="outline-none p-0 !z-[10000]"
        ...
        >
          <motion.div
            className="relative rounded-full overflow-hidden shadow-xl outline-none w-full h-full"
                        ...
                        >
                          <button
                            ...
                            onPointerDown={(e) => {
                              e.stopPropagation();
                              handleSelect(item);
                              // Trigger click on document to close menu
                              setTimeout(() => {
                                document.body.click();
                              }, 0);
                            }}
